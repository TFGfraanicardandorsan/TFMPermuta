class GenericValidators {
  ...
  static isString(valor, nombreCampo = "El campo", maxLength = 255) {
    ...
    if (typeof valor !== "string" || valor.trim() === "") {
      return {
        valido: false,
        mensaje: `${nombreCampo} debe ser un string no vacío`,
      };
    }
    if (valor.length > maxLength) {
      return {
        valido: false,
        mensaje: `${nombreCampo} no puede superar ${maxLength} caracteres`,
      };
    }
    return { valido: true };
  }
  ...
  static isFilePdfOrPng(fileId, nombreCampo = "Archivo", maxLength = 50) {
    ...
    if (
      ...
      return {
        valido: false,
        mensaje: `${nombreCampo} debe ser un archivo PDF o PNG`,
      };
    }
    return { valido: true };
  }
  ...
  static isArrayOfIntegers(valor, nombreCampo = "El campo") {
    ...
    for (const item of valor) {
      if (!Number.isInteger(item)) {
        return {
          valido: false,
          mensaje: `${nombreCampo} debe contener solo enteros`,
        };
      }
    }
    return { valido: true };
